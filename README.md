...
Here we introduce SNP Topic Modeling (SNPic), a representation learning framework that reformulates GWAS summary statistics by operating directly on discrete count matrices. Rather than operating on effect-size estimates, SNPic transforms summary statistics into document-term matrices, treating these matrices as a structured biological corpus. This is achieved through two complementary representation strategies: The \textit{Sumstat-as-word} scheme constructs a trait-trait matrix where each cell counts shared significant SNPs between a pair of phenotypes, capturing macroscopic pleiotropic relationships without requiring genomic annotations. The \textit{Gene-as-word} scheme maps SNPs to protein-coding genes, producing a trait-by-gene matrix that enables mechanistic interpretation at molecular resolution. Both representations are then decomposed by probabilistic topic models (Latent Dirichlet Allocation or Gaussian mixed-membership model), which serve as the inference engine within a broader, stability-optimized pipeline. Unlike factor models operating on continuous covariance matrices, this discrete formulation preserves the identity of individual genetic features throughout inference, so each latent topic remains directly interpretable through its constituent genes or variants. Bootstrap-based stability analysis replaces arbitrary hyperparameter selection with data-driven model choice. Together, these design choices shift GWAS interpretation from cataloging isolated associations toward decoding complex trait architectures as compositions of interpretable biological modules.

We validate SNPic through extensive simulations and analysis of large-scale human biobank datasets, demonstrating its ability to reconstruct biologically meaningful pleiotropic architectures across diverse human diseases. We further illustrate the framework's broad generalizability by recovering coherent genetic modules in model organisms and agricultural species, including \textit{Arabidopsis thaliana}, maize, and cattle. By translating isolated genetic signals into a structured, interpretable representation, SNPic enables systematic mapping of trait relationships beyond the resolution of correlation-based summaries, formalizing cross-trait GWAS comparison as a representation learning problem with direct biological interpretability.
